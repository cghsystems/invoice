	...
	public static void main(String[] args) {
		...
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document,
					new FileOutputStream("/home/chris/test.pdf"));
			...
			cell = new PdfPCell(new Phrase(
					"example cell with rowspan 2 and red border"));
			cell.setPadding(5);
			cell.setRowspan(2);
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			switch (writer.getPageNumber() % 2) {
				case 0:
					...
					Element.ALIGN_RIGHT, new Phrase("even header"), rect
					.getRight(), rect.getTop(), 0);
					break;
				case 1:
					ColumnText.showTextAligned(writer.getDirectContent(),
					Element.ALIGN_LEFT, new Phrase("odd header"), rect
					.getLeft(), rect.getTop(), 0);
					break;
			}
			ColumnText.showTextAligned(writer.getDirectContent(),
					Element.ALIGN_CENTER, new Phrase(String.format("page %d",
					writer.getPageNumber())), (rect.getLeft() + rect
					.getRight()) / 2, rect.getBottom() - 18, 0);
		}
	}
}
